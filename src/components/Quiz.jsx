    ...
    const startGame = () => {
        fetch(`${path}api/word`)
            ...
            .then((data) => {
                ...
                setSentenceTranslated(data.map((item) => item.translatedContextSentence));
        ...
        <div>
            ...
            {gameStarted && (
                <div>
                    ...
                    {hint1 && <div><strong>hint 1</strong>: {Array.from(answers[currentQuestionIndex]).map(char => (char === ' ' ? ' ' : '*')).join('')}</div>}
                    {hint2 && <div><strong>hint 2</strong>: {sentenceOriginal[currentQuestionIndex]}</div>}
