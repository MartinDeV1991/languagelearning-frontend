...
const Graph = ({ choice }) => {
    ...
    const [chartData, setChartData] = useState({
        ...
        datasets: [
            {
                label: choice === "correct" ? 'Number of Correct Guesses' : 'Number of Total Attempts',
                data: [],
            },
        ],
    });
    ...
    const updateChart = async () => {
        const dataWithLabels = rowData.map((item) => ({
            label: item.word,
            data: item.statistics ? (choice === "correct" ? item.statistics.guessedCorrectly : item.statistics.attempts) : 0,
        }));
    ...
    return (
        <div className="chart-container mb-5" style={{ width: '800px', height: '300px' }}>
            ...
            <Bar
                ...
                options={{
                    plugins: {
                        title: {
                            display: true,
                            text: `Number of ${choice === "correct" ? 'Correct Guesses' : 'Total Attempts'} by Word`,
                        },
                        legend: {
                            display: true,
                            position: 'top',
